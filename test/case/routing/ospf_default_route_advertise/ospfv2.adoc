=== OSPFv2 Default Route Advertise

ifdef::topdoc[:imagesdir: {topdoc}../../test/case/routing/ospf_default_route_advertise]

==== Description

Verify _default-route-advertising_ in OSPFv2, sometimes called 'redistribute
...
default-route-advertise, so R2 learns a default route via OSPFv2.  When
...
image::topology.svg[OSPFv2 Default Route Advertise topology, align=center, scaledwidth=75%]
...
. Verify R2 has a default route and R1's loopback from OSPFv2
. Verify connectivity from PC:data2 to R1's dummy interface
. Disable link PC:data1 <--> R1:data (take default gateway down)
. Verify R2 loses the default route but keeps R1's loopback from OSPFv2
