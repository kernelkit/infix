...
with infamy.Test() as test:
    ...
    with test.step("Wait for R1 and R2 to peer"):
        ...
        # Only that the peers have exchanged routes; which of the two parallel
        # links won is left to the traceroute below, since an OSPFv3 next-hop
        # cannot be matched against a configured address.
    ...
    with test.step("Wait for BFD sessions on the fast and slow links to come up"):
        ...
        # Counted rather than matched against the peer addresses, which are the
        # unpredictable link-local ones under OSPFv3.  Each router has BFD on
        # exactly the two links to its neighbor, so two sessions up is both of
        # them.
