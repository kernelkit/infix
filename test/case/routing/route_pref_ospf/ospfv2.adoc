=== Route preference: OSPFv2 vs Static

ifdef::topdoc[:imagesdir: {topdoc}../../test/case/routing/route_pref_ospf]

==== Description

This test configures a device with both an OSPFv2-acquired route on a
...
image::topology.svg[Route preference: OSPFv2 vs Static topology, align=center, scaledwidth=75%]
...
. Wait for OSPFv2 and static routes
. Verify connectivity from PC:data1 to PC:data2 via OSPFv2
