...
The upgrade of minor releases is expected to have low impact and
should be done as soon there is a patch release of Buildroot available.
...
5. Rebase the new branch onto the corresponding upstream release
   ```bash
   buildroot (2025.02.2-kkit)$ git rebase upstream/2025.02.2
   ```
...
The upgrade of minor releases is expected to have low impact and should be done as soon as
a patch release of the Linux kernel is available.
...
3. Fetch the latest tags from upstream
   ```bash
   linux (kkit-linux-6.12.y)$ git fetch upstream
   ```
...
5. Push changes and the tags
   ```bash
   linux (kkit-linux-6.12.y)$ git push -f --tags
   ```
