...
admin@example:/config/syslog/actions/log-file/file:file:/media/log/mylog/> set facility-list
...
they are "rotated".  The whole reason for this is not fill up the disk
with too old logs.  A rotated file is saved in stages and older ones are
also compressed (using `gzip`).  Use `show log` from admin-exec context
to start the log file viewer:
