...
> [!INFO]
> Interfaces and networks are mutually exclusive — attempting to configure
> both will result in a validation error.

Interfaces and networks serve very different purposes:
...
ensuring that no interface is left without firewall protection. This is
particularly important when adding new interfaces or when interfaces are
created dynamically (e.g., VLANs, bridges, or hotplugged devices).
