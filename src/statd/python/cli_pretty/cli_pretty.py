...
def show_wifi_scan(json, ifname):
    if not json.get("ietf-interfaces:interfaces"):
        print("Error, top level \"ietf-interfaces:interface\" missing")
        sys.exit(1)
    ...
    print(Decore.invert(hdr))
    iface = find_iface(json["ietf-interfaces:interfaces"]["interface"], ifname)
    if not iface:
...
def main():
    ...
    subparsers.add_parser('show-wifi-scan', help='Show scanning result on interface') \
    .add_argument('-n', '--name', help='Wi-Fi interface name')
    ...
    elif args.command == "show-wifi-scan":
        show_wifi_scan(json_data, args.name)
