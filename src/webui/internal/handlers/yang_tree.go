...
type leafGroupData struct {
	...
	// CanDelete is true for any non-presence container that currently
	// has data and isn't being rendered read-only. The template renders
	// a "Delete" button in the card header that DELETEs the whole
	// subtree — the only way out for non-presence containers with a
	// mandatory leaf (bridge-port, lag-port), and a generally useful
	// affordance for clearing IPv4/IPv6 etc. wholesale.
	CanDelete        bool
...
func (h *TreeHandler) fetchListInstances(r *http.Request, path string, listNode *schema.Node) []*schema.Node {
	...
	// Navigate directly to the list array using the full path. An absent key
	// in the RESTCONF response just means the list has no entries — silent
	// return, not an error.
...
func (h *TreeHandler) TreeNode(w http.ResponseWriter, r *http.Request) {
	...
	// When absent (direct navigation to a node), derive the immediate
	// parent path from the current path so the Back button and any
	// container-Delete action still know where to land.
	...
	if node.Kind == "container" || node.Kind == "list-instance" || isInstance {
		...
		if gd == nil && node.Presence == "" {
			...
			} else if !h.ReadOnly && node.Kind == "container" && parentPath != "" {
				// Any populated non-presence container can be wiped out
				// as a unit. Read-only Status view doesn't get the button.
				gd.CanDelete = h.checkPresenceExists(r, path)
			}
...
func (h *TreeHandler) buildLeafGroup(r *http.Request, mgr *schema.Manager, path, name, kind string) *leafGroupData {
	...
	for _, c := range children {
		...
		case "container":
			...
			// Don't inline NPC containers whose presence is signalled by
			// a mandatory leaf (bridge-port, lag-port). Those containers
			// represent a discrete "the interface is also a bridge port"
			// concept and deserve their own card — both for visual
			// consistency (bridge-port always shows as a sub-section
			// because of its nested flood/stp/multicast children, while
			// lag-port has only the `lag` leaf in candidate config) and
			// because their card header is where the Delete button lives.
			if err2 == nil && isSimpleList(kids2) && !hasMandatoryLeafChild(kids2) {
...
// hasMandatoryLeafChild reports whether any direct child node is a
// mandatory leaf or leaf-list — the YANG "implicit presence via
// mandatory leaf" idiom. Containers carrying one act as discrete
// sub-sections in the UI rather than being collapsed inline.
func hasMandatoryLeafChild(children []*schema.Node) bool {
	for _, c := range children {
		if (c.Kind == "leaf" || c.Kind == "leaf-list") && c.Mandatory {
			return true
		}
	}
	return false
...
// DeleteContainer serves DELETE /configure/tree/container?path=...&parent=...
// for any non-presence container the user wants to wipe out as a unit.
// Covers the NPC + mandatory leaf idiom (bridge-port, lag-port — where
// the leaf can't be cleared individually) and the generic "drop the
// whole IPv4 / IPv6 / DHCP block" case. After the deletion it re-renders
// the parent's leaf-group so the user lands on the parent view with
// the container gone.
//
// Implementation: rousette returns 502 on RESTCONF DELETE for non-presence
// containers (they have no resource to "remove"), so we GET the parent
// from the candidate, drop the target child from its JSON body, and PUT
// the parent back. PUT replaces the resource, which is the only RESTCONF
// idiom that actually removes an NPC subtree.
func (h *TreeHandler) DeleteContainer(w http.ResponseWriter, r *http.Request) {
	path := r.URL.Query().Get("path")
	parent := r.URL.Query().Get("parent")
	if path == "" || parent == "" {
		http.Error(w, "path and parent required", http.StatusBadRequest)
		return
	}
	mgr := h.Cache.Manager()
	if mgr == nil {
	...
	if err := h.deleteNPCContainer(r, mgr, path, parent); err != nil {
		http.Error(w, err.Error(), http.StatusBadGateway)
		return
	}
	...
	if err != nil || parentSchema == nil {
		http.Error(w, "parent schema lookup failed", http.StatusInternalServerError)
		return
	}
	kind := entryKindForGroup(parentSchema)
	gd := h.buildLeafGroup(r, mgr, parent, parentSchema.Name, kind)
	if gd == nil {
		gd = &leafGroupData{Path: parent, Name: parentSchema.Name, Kind: kind}
	}
	if grand := parentOfPath(parent); grand != "" {
		gd.ParentPath = grand
	}
	gd.ReadOnly = h.ReadOnly
	h.FragTmpl.ExecuteTemplate(w, "yang-leaf-group", gd)
...
// deleteNPCContainer removes the subtree at `path` from the candidate
// datastore by rewriting its parent: GET parent, strip the target key
// from the JSON, PUT parent. Required because rousette refuses DELETE
// on non-presence containers — they have no explicit resource handle.
func (h *TreeHandler) deleteNPCContainer(r *http.Request, mgr *schema.Manager, path, parent string) error {
	// Target's wrap key in JSON: last path segment, with any key
	// predicate stripped. Bridge-port's segment is "infix-interfaces:
	// bridge-port" — that's exactly what shows up as a key in the
	// parent's JSON object.
	targetKey := lastSegment(path)
	if i := strings.IndexByte(targetKey, '='); i >= 0 {
		targetKey = targetKey[:i]
	}
	if targetKey == "" {
		return fmt.Errorf("delete container: empty target segment")
	}

	raw, err := h.fetchData(r, parent)
	if err != nil {
		return fmt.Errorf("fetch parent %s: %w", parent, err)
	}
	var doc map[string]any
	if err := json.Unmarshal(raw, &doc); err != nil {
		return fmt.Errorf("decode parent %s: %w", parent, err)
	}

	// Locate the parent's data object within the wrapped response and
	// figure out the wrap key to use for the PUT body.
	parentObj, putWrap, err := unwrapForPut(doc, parent)
	if err != nil {
		return err
	}
	if _, ok := parentObj[targetKey]; !ok {
		return fmt.Errorf("delete container: %s not present in parent %s", targetKey, parent)
	}
	delete(parentObj, targetKey)

	var body map[string]any
	if isListInstancePath(parent) {
		body = map[string]any{putWrap: []any{parentObj}}
	} else {
		body = map[string]any{putWrap: parentObj}
	}
	return h.RC.Put(r.Context(), candidateDS+parent, body)
}

// unwrapForPut peels the RESTCONF GET envelope until it lands on the
// parent's own data object. For a container path it descends one level
// (outer container → object). For a list-instance path it descends two
// (outer container → list array → entry[0]). The returned wrap key is
// the qualifier the PUT body needs at the parent path: container-name
// for a container, list-name for a list-instance.
func unwrapForPut(doc map[string]any, parentPath string) (map[string]any, string, error) {
	_, outerVal, err := singleEntry(doc)
	if err != nil {
		return nil, "", fmt.Errorf("unwrap parent: %w", err)
	}
	if !isListInstancePath(parentPath) {
		obj, ok := outerVal.(map[string]any)
		if !ok {
			return nil, "", fmt.Errorf("unwrap parent: container value not an object")
		}
		// The wrap key for a container PUT is the container's own qualified
		// name, which is also the outer map's only key.
		var key string
		for k := range doc {
			key = k
		}
	...
	inner, ok := outerVal.(map[string]any)
	if !ok {
		return nil, "", fmt.Errorf("unwrap parent: list container value not an object")
	}
	listKey, arr, err := singleEntry(inner)
	if err != nil {
		return nil, "", fmt.Errorf("unwrap list: %w", err)
	}
	entries, ok := arr.([]any)
	if !ok || len(entries) == 0 {
		return nil, "", fmt.Errorf("unwrap list: empty or non-array value")
	}
	entry, ok := entries[0].(map[string]any)
	if !ok {
		return nil, "", fmt.Errorf("unwrap list: first entry not an object")
	}
	return entry, listKey, nil
}

// singleEntry returns the only key/value in m, or an error when m has
// none or more than one — RESTCONF GET responses are always wrapped in
// a single top-level qualifier, so any other shape is bug-worthy.
func singleEntry(m map[string]any) (string, any, error) {
	if len(m) != 1 {
		return "", nil, fmt.Errorf("expected single entry, got %d", len(m))
	}
	for k, v := range m {
		return k, v, nil
	}
...
func isListInstancePath(p string) bool {
	return strings.ContainsAny(lastSegment(p), "[=")
}
...
// entryKindForGroup maps a schema node to the display Kind expected by
// the leaf-group template ("list-instance" when the node carries keys,
// "container" otherwise).
func entryKindForGroup(n *schema.Node) string {
	if len(n.Keys) > 0 {
...
// parentOfPath strips the last segment from a RESTCONF path. Returns ""
// when there is no parent (root or a single segment).
...
// When the leaf carries a `must "deref(.)/../<sibling>"` constraint, the
// result is filtered to entries whose parent object exposes that sibling
// — the YANG idiom for "this leafref must point at an object of kind X".
func (h *TreeHandler) fetchLeafrefValues(r *http.Request, mgr *schema.Manager, node *schema.Node) []string {
	...
	// Split off the target leaf name, then walk up the schema until we
	// hit a non-list ancestor — RESTCONF refuses to GET a bare list path
	// without a key selector ("List '…' requires 1 keys"), so the actual
	// fetch has to target the list's enclosing container. The collector
	// below recurses into the container and pulls the leaf value out of
	// every list entry.
	...
	for len(parentSegs) > 0 {
		candidate := "/" + strings.Join(parentSegs, "/")
		n, err := mgr.NodeAt(candidate)
		if err != nil || n == nil || n.Kind != "list" {
			break
		}
	...
	if node.Type.LeafrefSibling != "" {
		return extractFieldValuesWithSibling(data, targetLeaf, node.Type.LeafrefSibling)
	}
...
// extractFieldValuesWithSibling is like extractFieldValues but only keeps
// matches where the same parent object also has a child whose key strips
// to siblingName. Implements the must "deref(.)/../<sibling>" filter
// idiom for leafref dropdowns.
func extractFieldValuesWithSibling(data []byte, fieldName, siblingName string) []string {
	var raw interface{}
	if err := json.Unmarshal(data, &raw); err != nil {
		return nil
	}
	var vals []string
	collectFieldWithSibling(raw, fieldName, siblingName, &vals)
	sort.Slice(vals, func(i, j int) bool { return naturalLess(vals[i], vals[j]) })
	return vals
}

func collectFieldWithSibling(v interface{}, field, sibling string, out *[]string) {
	switch t := v.(type) {
	case map[string]interface{}:
		var fieldVal string
		var haveField, haveSibling bool
		for k, child := range t {
			_, local := splitModPrefix(k)
			switch local {
			case field:
				if s, ok := child.(string); ok {
					haveField, fieldVal = true, s
				}
			case sibling:
				haveSibling = true
			}
		}
		// Same record-boundary rule as collectField: if the field is
		// present at THIS level, stop here. Append only when the sibling
		// is also present; otherwise skip without descending so we don't
		// pick up an inner object's field as a spurious match.
		if haveField {
			if haveSibling {
		...
		for _, child := range t {
			collectFieldWithSibling(child, field, sibling, out)
		}
	case []interface{}:
		for _, item := range t {
			collectFieldWithSibling(item, field, sibling, out)
		}
	}
}
...
func collectField(v interface{}, field string, out *[]string) {
	switch t := v.(type) {
	case map[string]interface{}:
		// First pass: if the field is present at THIS level, treat the
		// map as a leafref-target record and extract here without
		// recursing into siblings. This stops spurious matches from
		// nested objects that happen to share the field name — e.g. an
		// asymmetric-key entry has its own `name` AND a nested
		// `certificates/certificate[]/name`; only the entry-level name
		// is a valid leafref target.
		for k, child := range t {
			_, local := splitModPrefix(k)
			if local == field {
				if s, ok := child.(string); ok {
		...
		// Field not at this level — descend.
		for _, child := range t {
