...
static int change_clock(sr_session_ctx_t *session, struct lyd_node *config, struct lyd_node *diff, sr_event_t event, struct confd *confd)
{
	...
	if (writesf(timezone, "w", TIMEZONE_NEXT) || chmod(TIMEZONE_NEXT, 0644)) {
...
static int change_ntp_client(sr_session_ctx_t *session, struct lyd_node *config, struct lyd_node *diff, sr_event_t event, struct confd *confd)
{
	...
	for (size_t i = 0; i < cnt; i++) {
		...
		fp = fopenfp(0640, "chrony", "/etc/chrony/sources.d/%s.sources", name);
		if (!fp) {
	...
	if (srx_enabled(session, XPATH_NTP_"/enabled")) {
		...
		FILE *fp = fopenp(NTP_CLIENT_CONF, 0640, "chrony");
...
static sr_error_t generate_auth_keys(sr_session_ctx_t *session, const char *xpath)
{
	...
	LYX_LIST_FOR_EACH(lyd_child(auth), user, "user") {
		...
		/* sshd reads authorized keys with the user's privileges */
		pw = getpwnam(username);
		if (!pw || fchown(fileno(fp), pw->pw_uid, 0) || fchmod(fileno(fp), 0640))
			ERRNO("failed setting owner of user \"%s\" authorized_keys file", username);
...
static int change_motd(sr_session_ctx_t *session, struct lyd_node *config, struct lyd_node *diff, sr_event_t event, struct confd *confd)
{
	...
	if (message) {
		rc = writesf(message, "w", "%s", fn) || chmod(fn, 0644);
		free(message);
	}
...
static int write_binary(const char *fn, mode_t mode, const char *what, const char *data)
{
	...
	if (fchmod(fd, mode))
		ERRNO("failed setting mode on %s", fn);
