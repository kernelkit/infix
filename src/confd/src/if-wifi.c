...
static wifi_mode_t wifi_get_mode(struct lyd_node *wifi)
{
	if (lydx_get_child(wifi, "station"))
		return wifi_station;
	else if (lydx_get_child(wifi, "access-point"))
		return wifi_ap;
	return wifi_unknown;
}
...
int wifi_mode_changed(struct lyd_node *wifi)
{
	...
	ERROR("Mode changed: %s", ((station && station_op == LYDX_OP_DELETE) || (ap && ap_op == LYDX_OP_DELETE) == 1 ? "yes" : "no"));
...
int wifi_add_iface(struct lyd_node *cif, struct dagger *net)
{
	...
	DEBUG("Creating WiFi interface %s on radio %s (type: %s)",
	      ifname, radio, wifi_get_mode(wifi) == wifi_station ? "station" : "ap");
