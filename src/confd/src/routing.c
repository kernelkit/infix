...
int parse_rip_interfaces(sr_session_ctx_t *session, struct lyd_node *interfaces, FILE *fp)
{
	...
	/* Output network and passive-interface statements in netd format */
	LY_LIST_FOR(lyd_child(interfaces), interface) {
		...
		/* netd format: simple "network IFNAME" and "passive IFNAME" */
	...
	/* Note: Per-interface settings like split-horizon, send/recv version
	 * are not yet supported in the netd simple format. These would need
	 * to be added to the netd RIP parser if required. */
...
int parse_rip_timers(sr_session_ctx_t *session, struct lyd_node *timers, FILE *fp)
{
	...
	/* netd format: "timers update N invalid N flush N" */
...
{
	struct lyd_node *interfaces, *timers, *default_route;
	const char *default_metric, *distance;
	int num_interfaces = 0;

	/* Write [rip] section header for netd */
	fputs("\n[rip]\n", fp);
...
static int parse_static_routes(sr_session_ctx_t *session, struct lyd_node *parent, FILE *fp)
{
	...
	/* Write [routes] section header for netd */
	fputs("[routes]\n", fp);
