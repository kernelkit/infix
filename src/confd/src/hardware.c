...
static int wifi_gen_station(const char *ifname, struct lyd_node *station,
			     const char *radio, struct lyd_node *config, struct dagger *net)
{
...
static int wifi_gen_aps_on_radio(const char *radio_name, struct lyd_node *cifs,
				  struct lyd_node *radio_node, struct lyd_node *config, struct dagger *net)
{
	const char *ssid, *hidden, *security_mode, *secret_name, *secret;
	...
	if (secret_name && strcmp(security_mode, "open") != 0) {
		...
		if (secret_node) {
			ERROR("Found secret node, type: %d, name: %s", secret_node->schema->nodetype, secret_node->schema->name);
			secret = lyd_get_value(secret_node);
			ERROR("Secret value: %s", secret ? "***" : "NULL");
		}
		if (!secret) {
	...
	/* PHY mode configuration - enable modern modes based on band */
		...
		if (!strcmp(class, "infix-hardware:usb")) {
			...
			if (wifi_iface_count == 1 && station) {
				...
				if (station && lydx_is_enabled(iface, "enabled")) {
					...
					rc = wifi_gen_station(ifname, station, name, config, &confd->netdag);
					if (rc != SR_ERR_OK) {
				...
				rc = wifi_gen_aps_on_radio(name, interfaces_config, cwifi_radio, config, &confd->netdag);
				if (rc != SR_ERR_OK) {
