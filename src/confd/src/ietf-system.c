...
static int set_password(struct confd *confd, const char *user, const char *hash, bool lock)
{
	...
	if (!strncmp(hash, factory, strlen(factory))) {
		...
		if (!json_is_string(pwd)) {
			 ...
			 */
			SECURITY("%s: cannot find factory-default password hash!", user);
			lock = true;
		} else {
...
static sr_error_t handle_sr_passwd_update(sr_session_ctx_t *, struct confd *confd, struct sr_change *change)
{
	...
	case SR_OP_MODIFIED:
		 ...
		 */
		if (!hash || !strlen(hash)) {
			SECURITY("Empty passwords are not allowed"
				 ", disabling password login for user %s.", user);
			hash = "*";
		}
	...
	case SR_OP_DELETED:
		if (set_password(confd, user, "*", true))
			...
			SECURITY("Password deleted for user %s", user);
		break;
	case SR_OP_MOVED:
		break;
	}
