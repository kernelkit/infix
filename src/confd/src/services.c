...
/*
 * A newline in a value injects a directive into the snippet.  The
 * models reject it, this is the second line of defence.
 */
static bool tftp_sane(struct lyd_node *tftp)
{
	struct lyd_node *iface;

	if (strpbrk(lydx_get_cattr(tftp, "root") ?: "", "\r\n"))
		return false;

	LYX_LIST_FOR_EACH(lyd_child(tftp), iface, "interface") {
		if (strpbrk(lyd_get_value(iface) ?: "", "\r\n"))
			return false;
	}
...
static int tftp_change(sr_session_ctx_t *session, struct lyd_node *config, struct lyd_node *diff, sr_event_t event, struct confd *confd)
{
	...
	if (!tftp_sane(tftp)) {
		ERROR("refusing TFTP settings with a newline in them");
